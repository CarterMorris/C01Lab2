...
app.delete("/deleteNote/:noteId", express.json(), (req, res) => {
  try {
    // Basic param checking
    const noteId = req.params.noteId;
    if (!ObjectId.isValid(noteId)) {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (!data) {
        return res
          .status(404)
          .json({ error: `Note with ID ${noteId} was not found.` });
      }
  ...
  try {
    ...
    const { title, content } = req.body;
      ...
      if (!title && !content) {
        return res
          .status(400)
          .json({ error: "New title and or content required to update note." });
      }
